...
> ⚠️ **ТРЕБУЕТ УТОЧНЕНИЯ:** Является ли поле «тема» обязательным для перехода на шаг 2, или можно перейти только с загруженными файлами?
...
> ⚠️ **ТРЕБУЕТ УТОЧНЕНИЯ:** Кнопка «Далее» не видна на текущем скриншоте (вероятно, в нижней части страницы). Её состояния (active/disabled/loading) требуют уточнения у дизайнера.

> ⚠️ **ТРЕБУЕТ УТОЧНЕНИЯ:** Обработка `file_id` при переходе между шагами — хранится в React state, Zustand или URL-параметрах?

> ⚠️ **ТРЕБУЕТ УТОЧНЕНИЯ:** Отображение статуса индексации файла (`processing → ready`) на шаге 1 — показывается ли спиннер/прогресс-бар напротив каждого файла?
